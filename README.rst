...
Why bother building Plock on top of Buildout? Plock is a work around for ``pip install Plone`` which works but requires a lengthy requirements.txt, and lacks additional conveniences provided by Buildout that must be replaced in order for ``pip install Plone`` to work properly.
